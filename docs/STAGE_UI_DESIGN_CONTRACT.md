...
- Stage 1 hands off the **research selection** by writing `spot.stage01_selection.v1` (see `_frontend/src/domain/selection.ts`) to the single versioned `localStorage` key `spot.stage01_selection.v1` and to `window.__stage01SelectionArtifact`, then navigating to `/02_page.html#/stage-2`. The shell reads that key on load.
- Namespace/production discipline: the Stage-1 bridge emits `namespace:"research_only"`, `production_gate_passed:false`. The shell must render research vs production provenance chips (`research_only` = amber) and must never relabel a research artifact as production.
