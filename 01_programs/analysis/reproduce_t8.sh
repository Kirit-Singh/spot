...
set -euo pipefail
cd "$(dirname "$0")"
echo "[1/7] integrate primary-source provenance"; python3 gen_stage1_provenance.py
echo "[2/7] independent provenance verifier";     python3 verify_stage1_provenance.py
echo "[3/7] generate T8 layer";                   python3 gen_stage1_t8.py
echo "[4/7] independent T8 verifier";             python3 verify_stage1_t8.py
echo "[5/7] protected-baseline gate (Stage-2 view + scorer-projection + no Tier-2 leak)"; python3 stage2_bridge/protected_hashes.py
echo "[6/7] mutation / forgery suites";           python3 -m pytest test_stage1_t8.py test_stage1_provenance.py stage2_bridge/tests -q
echo "[7/7] full-release attestation";            python3 gen_full_release_verification.py
