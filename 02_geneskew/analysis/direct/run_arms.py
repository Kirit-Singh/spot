...
def base_deltas(*, ctx: dict[str, Any], args, cond: str, admitted: list[str],
    ...
    ``signature_targets`` asks for the target-masked SIGNATURE of those targets — the full
    effect vector on the target's own unmasked support — for the PATHWAY lane, which calls
    this function by name. It is built from the very mask the deltas are taken under, so a
    signature can never be masked differently from the numbers it is meant to explain.
    """
    release = ctx["release"]
    ...
    for i, target in enumerate(targets):
        ...
        # THE TARGET-MASKED SIGNATURE (pathway lane), under the SAME mask the deltas are
        # taken under. A signature masked differently from the numbers it explains would be
        # explaining different numbers.
    ...
    return {"base": out, "signatures": signatures, "mask_rows": mask_rows,
            "contrib_rows": contrib_rows, "guide_rows": guide_rows,
            "donor_rows": donor_rows, "n_source_targets": len(targets)}
...
def build_bundle(args) -> dict[str, Any]:
    ...
    scanned = base_deltas(ctx=ctx, args=args, cond=cond, admitted=admitted)
    base = scanned["base"]
    ...
    #
    # INTEGRATION: called through `rs.bundle_input_manifest`, which is the name W18's line
    # introduced, and which now delegates to `arm_inputs` — the richer manifest that also
    # binds the guide manifest, source registry, target-identity map, donor crosswalk,
    # strict-replay source and pseudobulk. One implementation behind one name; two would drift.
    ...
    binding = {
        ...
        #
        # INTEGRATION: through `rs.contributor_manifest_identity` — W18's name — which now
        # carries W14's full semantic block as well as the raw bytes. Both lines bound this
        # manifest; neither binding is dropped.
        ...
        # THE SOLVER LOCK (W18), VERIFIED against the pin and bound by its FULL sha256.
        ...
        # refused. Recording the lock beside a run says which environment the producer HAD;
        # binding it INTO the run id says which environment the numbers CAME FROM. Only the
        # second survives a swap, so a missing or swapped lock REFUSES.
