...
-------------------------
``question_id``, ``selection_id``, the pole/endpoint identities and the declared method
identity are DERIVED in ``stage1_v3_ids`` — one job, one module, readable on its own. They
are re-exported here (see the import below), because this module is the public surface every
lane and every test names them through. This module GATES; that one DERIVES.
"""
...
# THE IDENTITIES, derived in one place (``stage1_v3_ids``) and RE-EXPORTED here.
#
# Identity is one job; the gate is another. The derivations moved out so they can be read and
# audited on their own — but they are re-exported under their original names because every
# caller, every lane and every test names them through this module, and an identity that
# changed its import path would be an identity that changed. The names below ARE the public
# surface of this module; ``stage1_v3_ids`` is where they are defined.
from .stage1_v3_ids import (  # noqa: F401  (re-exported: this module is the public surface)
    ENDPOINT_RULE,
    ENDPOINT_RULE_ID,
    METHOD_IDENTITY_BINDING,
    METHOD_IDENTITY_FIELDS,
    METHOD_IDENTITY_KIND,
    METHOD_IDENTITY_NOT_A,
    METHOD_IDENTITY_NOT_REDERIVED_BECAUSE,
    METHOD_IDENTITY_REDERIVATION_OWNER,
    METHOD_IDENTITY_RULE_ID,
    POLE_IDENTITY_RULE,
    POLE_IDENTITY_RULE_ID,
    QUESTION_ID_LEN,
    QUESTION_ID_RULE,
    QUESTION_ID_RULE_ID,
    SELECTION_ID_LEN,
