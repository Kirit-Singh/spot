...
def build_bundle(root: str, lane: str, ctx: dict, staged: dict,
    ...
    inv: dict[str, Any] = {
        "fixture": True,
        # The Direct lane wears W10's NATIVE bundle schema so W3's typed adapter
        # (verify_lane_admission.adapt_direct) can read `shipped` from it: it keys on
        # exactly this schema + a bare top-level `condition` + `arm_bundle_run_id`. The
        # generic `lane`/`context`/`bundle_id` are kept ALONGSIDE so the (generic-shape)
        # per-bundle scanner and the inventory reader still read the same slot.
        "schema_version": ("spot.stage02_direct_arm_bundle.v1" if lane == "direct"
                           else "spot.stage02_arm_bundle.v1"),
        "lane": lane,
        "context": ctx,
        "stage1_v3_release": {
    ...
    if lane == "direct":
        inv["condition"] = ctx["condition"]        # W10's bare top-level condition
    if lane == "pathway":
    ...
    _nid = f"FIXTURE-{_canon(inv)[:16]}"
    inv["bundle_id"] = _nid
    if lane == "direct":
        inv["arm_bundle_run_id"] = _nid            # W10's native Direct identity
...
def write_external_admission(run: dict, release_id=None, verdict="ADMIT",
    ...
    doc = {
        "fixture": True,
        "schema_version": NATIVE_ADMISSION[lane]["schema_version"]
                          if lane != "direct" else LANE_VERIFIERS[lane]["schema_version"],
...
# --------------------------------------------------------------------------- #
NATIVE_ADMISSION = {
    # W1's REAL Direct binding: ONE FLAT FILE PER CONDITION, admitting a BUNDLE. The
    # producer's direct_release.json is never touched — it ships pending and stays pending;
    # the LANE admission is W3's typed per-condition binding, read by adapt_direct.
    "direct": {
...
def write_native_admission(run: dict, lane: str, *, verdict="ADMIT", admitted=True,
    ...
    root = run["root"]
    if lane == "direct":
        # W1's REAL shape: ONE FLAT BINDING PER CONDITION. W10 admits a BUNDLE at a time,
        # and W1's adapter normalizes each native report into `direct_admission_<condition>
        # .json`. There is no single `direct_release_admission.json`, and `code_identity` is
        # a STRING. The exact fields are W3's verify_lane_admission.adapt_direct contract.
