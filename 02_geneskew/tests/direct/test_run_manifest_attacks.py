...
class TestAGeneSetSourceNameIsNotAGeneSetIdentity:
    ...
    def _forge(run, source, **fields):
        for d in run["pathway"]:
            ...
            if BS.normalize(json.load(open(os.path.join(d, "arm_bundle.json"))))["context"]["gene_set_source"] != source:
                continue
...
class TestGate20TheNativeVerdictIsTypedNotTransliterated:
    ...
    def test_the_NATIVE_uppercase_ADMIT_is_accepted_AS_IT_IS(self, tmp_path):
        ...
        # W1's binding admits a BUNDLE, so the verifier is W10's BUNDLE verifier — not a
        # release verifier. There is no `direct_release_admission.json`.
