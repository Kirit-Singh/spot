...
import { deriveExecutionStatus, SelectionError, verifySelectionV3 } from '../selectionV3';
...
async function build(opts: BuildOpts = {}) {
  ...
  const contract: Record<string, unknown> = {
    ...
    execution_status: opts.execution_status ?? deriveExecutionStatus(mode, aStatus, bStatus),
    ...
    estimator_status: opts.estimator_status ?? (mode === 'within_condition' ? 'available' : 'not_implemented'),
...
describe('selection.v3 — valid routing resolves', () => {
  ...
  it('temporal_cross_condition → awaiting_estimator (never ready)', async () => {
    const r = await verifySelectionV3(await build({ mode: 'temporal_cross_condition' }));
...
describe('selection.v3 — routing semantics enforced, not trusted', () => {
  it('rejects temporal labelled ready', async () => {
    await expectCode(() => build({ mode: 'temporal_cross_condition', execution_status: 'ready' }).then(verifySelectionV3), 'temporal_labeled_ready');
  });
