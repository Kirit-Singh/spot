...
This module exists to retire one line — ``arm_query``'s source-code Boolean, which was
Stage-3's production gate and was never a gate at all (audit B2). A constant in a Python
file means a REAL Stage-2 aggregate, independently admitted on disk, is still refused
because nobody edited the file — and that flipping it to ``True`` would admit ANYTHING,
because it names no manifest, no report, no verifier and no bytes. A `True` in a source
file is a developer's memory of a report they once read. It is not the report.
...
THE MANIFEST MUST PROVE ITS OWN IDENTITY (B6, again)
----------------------------------------------------
``manifest_sha256`` is recomputed here from the manifest's own semantic content (excluding
that field and the non-semantic timestamps). A manifest that cannot recompute its own
identity is not a root of trust — it is a document asserting a number.

The two hashes answer different questions: the **self-hash** asks "is this manifest
internally what it says it is?"; the **raw/canonical** pair asks "is this the manifest the
verifier actually read?" — raw catches a re-serialisation, canonical catches a re-format,
and the report must bind BOTH or it is an opinion about some other artifact.

Bundles are verified by their RAW bytes against the inventory. Their declared canonical
hashes are required (an unaddressable bundle is not admissible) and carried, but not
recomputed: Stage 2 owns that rule, its arm values are floats on the wire, and Stage-3's
``hashing.canonical_json`` deliberately refuses floats — recomputing under a rule the pin
was not computed under would fail for the WRONG reason, and a wrong-reason failure teaches
the next reader to add an exception. Raw bytes are unambiguous, and strictly stronger.

TOPOLOGY IS DERIVED, NEVER DECLARED
-----------------------------------
The release is EXACTLY 15 physical bundles — 3 Direct (one per condition), 6 temporal (one
per ORDERED pair; Rest→Stim48hr is not Stim48hr→Rest), 6 pathway (condition x gene-set
source) — carrying 300 logical arm slots (10 programs x 2 desired changes x 15 contexts).
Every one of those numbers is derived here from the conditions and sources, never read
from a declared count: a producer that can declare its own completeness can declare a
PARTIAL release complete, and a missing bundle then looks exactly like a bundle that was
computed and found empty.

It assigns **no pair roles** (``away_from_A``/``toward_B``) — a role is what a *selection*
gives an arm at join time, and baking one in fuses two questions under one key — and emits
**no combined score**. It loads reusable arms, and nothing else.
"""
...
AGGREGATE_MANIFEST_SCHEMA = "spot.stage02_aggregate_run_manifest.v1"
AGGREGATE_REPORT_SCHEMA = "spot.stage02_aggregate_verification.v1"

# --------------------------------------------------------------------------- #
# The topology. Derived from these, never from a declared count.
# --------------------------------------------------------------------------- #
...
# The manifest's own identity, and the fields that are NOT part of it. A timestamp is not
# semantic content: two runs of the same release differ in when they were written.
...
# --------------------------------------------------------------------------- #
# Named gates. Every refusal below names one of these, so a refusal can be grepped,
# tested and cited — never inferred from a message someone later rewords.
# --------------------------------------------------------------------------- #
...
GATE_VERIFIER_NOT_INDEPENDENT: str = "verification_report_is_not_from_an_independent_verifier"
...
class LoadedArm:
    ...
    def slot(self) -> tuple[str, str, str]:
        return (self.bundle_key, self.program_id, self.desired_change)
...
class AdmittedAggregate:
    ...
    def arms_in(self, lane: str) -> list[LoadedArm]:
        return [a for a in self.arms if a.lane == lane]


# --------------------------------------------------------------------------- #
# 1. Identity. The manifest proves who it is, or it is not a root of trust.
# --------------------------------------------------------------------------- #
def _load_json(path: str, what: str) -> tuple[dict[str, Any], str]:
    ...
    if not os.path.isfile(path):
        _refuse(GATE_ARTIFACT_NOT_ON_DISK,
                f"the {what} is not on disk at {path!r}. There is no fixture fallback: a "
                "Stage-3 run without its admitted Stage-2 aggregate does not quietly "
...
def manifest_self_hash(manifest: dict[str, Any]) -> str:
    """The manifest's identity, over its OWN semantic content.

    Excludes the self-hash field (it cannot hash itself) and the non-semantic timestamps
    (two writes of the same release differ only in when they happened).
    """
    try:
        return content_hash(without(manifest, (SELF_HASH_FIELD, *NON_SEMANTIC_FIELDS)))
    except CanonicalizationError as exc:
        _refuse(GATE_MANIFEST_UNREADABLE,
                f"the aggregate manifest cannot be canonically hashed: {exc}. A manifest "
                "carrying a float carries a magnitude no two readers will agree on.")
...
def _check_manifest_identity(manifest: dict[str, Any]) -> str:
    ...
    if not declared:
        _refuse(GATE_MANIFEST_SELF_HASH,
                f"the aggregate manifest declares no {SELF_HASH_FIELD}. A manifest that "
                "does not commit to its own identity cannot be the root of anything.")
    if declared != recomputed:
        _refuse(GATE_MANIFEST_SELF_HASH,
                f"the manifest declares {SELF_HASH_FIELD}={str(declared)[:16]}… but its "
                f"own content hashes to {recomputed[:16]}…. It does not prove its own "
                "identity, so nothing it says about anything else can be trusted.")
    return recomputed
...
def _check_report(report: dict[str, Any], *, manifest_raw: str,
    ...
    if "independent" not in verifier_id:
        _refuse(GATE_VERIFIER_NOT_INDEPENDENT,
                f"the verification report names verifier {verifier_id!r}, which is not an "
                "INDEPENDENT verifier. A producer agreeing with itself is the one thing an "
                "independent verifier exists to rule out.")

    verdict = report.get("verdict")
    if verdict != ADMIT:
        _refuse(GATE_VERDICT_NOT_ADMIT,
                f"the independent verifier's verdict is {verdict!r}, not {ADMIT!r}. Stage 3 "
                "consumes an admitted aggregate or none.")
    ...
    if raw != manifest_raw or canonical != manifest_canonical:
        _refuse(GATE_REPORT_BINDS_ANOTHER_MANIFEST,
                "the report admits a DIFFERENT manifest than the one on disk:\n"
                f"  report admits raw={str(raw)[:16]}… canonical={str(canonical)[:16]}…\n"
                f"  manifest is    raw={manifest_raw[:16]}… "
                f"canonical={manifest_canonical[:16]}…\n"
                "Both must match: raw alone would miss a re-serialisation, canonical alone "
                "would let the shipped file differ from what was judged.")
...
def _check_stage1_release(manifest: dict[str, Any], stage1_release_path: str) -> str:
    ...
    if not pinned:
        _refuse(GATE_STAGE1_RELEASE_UNBOUND,
                "the manifest pins no Stage-1 release (stage1_release.raw_sha256). An "
                "aggregate that cannot name the release it was computed against cannot be "
                "replayed against it.")
    ...
    if on_disk != pinned:
        _refuse(GATE_STAGE1_RELEASE_UNBOUND,
                f"the Stage-1 release on disk hashes to {on_disk[:16]}… but the aggregate "
                f"manifest pins {str(pinned)[:16]}…. The aggregate was computed against a "
                "different release than the one supplied.")
    return on_disk


# --------------------------------------------------------------------------- #
# 2. Topology. 15 bundles, derived. 300 arm slots, derived.
# --------------------------------------------------------------------------- #
...
def _entry_key(entry: dict[str, Any]) -> str:
    """The bundle key IMPLIED by the entry's own context. A key that disagrees with its
    context is a mislabelled bundle, and a mislabelled bundle fills the wrong slot."""
    lane = entry.get("lane")
    if lane == LANE_DIRECT:
        return f"{LANE_DIRECT}|{entry.get('condition')}"
    if lane == LANE_TEMPORAL:
        return (f"{LANE_TEMPORAL}|{entry.get('from_condition')}"
                f"|{entry.get('to_condition')}")
    if lane == LANE_PATHWAY:
        return f"{LANE_PATHWAY}|{entry.get('condition')}|{entry.get('pathway_source')}"
    return ""
...
def _safe_bundle_path(rel_path: Any, bundles_root: str) -> str:
    ...
    if os.path.isabs(rel_path):
        _refuse(GATE_PATH_TRAVERSAL,
                f"bundle path {rel_path!r} is ABSOLUTE. Every bundle in the release lives "
                "under the explicit bundles root; an absolute path reads a file the "
                "admission never bound.")
    if ".." in rel_path.replace("\\", "/").split("/"):
        _refuse(GATE_PATH_TRAVERSAL,
                f"bundle path {rel_path!r} traverses upwards ('..'). It would leave the "
                "root the independent report admitted.")

    root = os.path.realpath(bundles_root)
    full = os.path.realpath(os.path.join(root, rel_path))
    if full != root and os.path.commonpath([root, full]) != root:
        _refuse(GATE_PATH_TRAVERSAL,
                f"bundle path {rel_path!r} resolves to {full!r}, which is OUTSIDE the "
                f"bundles root {root!r} (a symlink counts: realpath is what is opened).")
    return full
...
def _check_inventory(manifest: dict[str, Any],
    ...
    if not isinstance(inventory, list) or not inventory:
        _refuse(GATE_INCOMPLETE_TOPOLOGY,
                ...
                f"{N_BUNDLES} bundles. A missing inventory is a partial release, and a "
                "partial release is never admissible.", topology=True)
    ...
    for entry in inventory:
        ...
        if lane not in LANES or key not in expected or expected[key] != lane:
            _refuse(GATE_UNKNOWN_LANE,
                    f"inventory entry {key!r} (lane={lane!r}) is not one of the "
                    f"{N_BUNDLES} bundles this release has. The release is derived from "
                    f"conditions {list(CONDITIONS)} and sources {list(PATHWAY_SOURCES)}; an "
                    "unknown lane or context is a bundle from another release.")
        if key != _entry_key(entry):
            _refuse(GATE_UNKNOWN_LANE,
                    f"inventory entry declares bundle_key {key!r} but its own context "
                    f"implies {_entry_key(entry)!r}. A mislabelled bundle fills the wrong "
                    "slot, and the slot it should have filled then looks merely empty.")
        if key in seen:
            _refuse(GATE_DUPLICATE_BUNDLE,
                    f"bundle_key {key!r} appears twice in the inventory. A duplicate "
                    "silently fills a missing slot, and the count still looks right.")
        if not (entry.get("raw_sha256") and entry.get("canonical_sha256")):
            _refuse(GATE_BUNDLE_BYTES_MOVED,
                    f"bundle {key!r} is not content-addressed (raw_sha256 + "
                    "canonical_sha256); a bundle nobody can address is a bundle nobody "
                    "admitted.")
        ...
        seen[key] = bundle
        out.append((bundle, full))

    missing = sorted(set(expected) - set(seen))
    if missing:
        _refuse(GATE_MISSING_BUNDLE,
                f"the inventory is missing {len(missing)} bundle(s): {missing}. The "
                f"release is exactly {N_BUNDLES} physical bundles — 3 Direct, 6 ordered "
                "temporal pairs, 6 pathway condition x source — and a missing bundle is "
                "indistinguishable from one that was computed and found empty.",
                topology=True)
    if len(out) != N_BUNDLES:
        _refuse(GATE_INCOMPLETE_TOPOLOGY,
                f"the inventory resolves {len(out)} bundles; the release is exactly "
                f"{N_BUNDLES}.", topology=True)
    return out
    ...
    for rec in arm.get("records") or ():
        ...
        if lane in MEASURED_LANES and base is None:
            _refuse(GATE_ARM_IDENTITY_UNRESOLVED,
                    f"arm {arm_key!r} record points at base_key {rec.get('base_key')!r}, "
                    "which resolves to nothing. Referential integrity is not optional: an "
                    "unresolvable record is a drug target nobody can name.")
        base = base or {}
        if base and base.get("target_id") != rec.get("target_id"):
            _refuse(GATE_ARM_IDENTITY_UNRESOLVED,
                    f"arm {arm_key!r}: base_key {rec.get('base_key')!r} resolves to target "
                    f"{base.get('target_id')!r} but the record says "
                    f"{rec.get('target_id')!r}. The join is checked, never trusted.")
        if lane == LANE_TEMPORAL:
            estimate: Any = {"from": base.get("from_released_estimate_id"),
                             "to": base.get("to_released_estimate_id")}
        else:
            estimate = base.get("released_estimate_id")
        rows.append({
            ...
            # A rank of null is a STATE (unranked), not an absence. It never becomes 0,
            # and it never becomes last.
            "rank": rec.get("rank"),
...
def _load_bundle_arms(bundle: AdmittedBundle, full_path: str, *,
    ...
    if raw != bundle.raw_sha256:
        _refuse(GATE_BUNDLE_BYTES_MOVED,
                f"bundle {bundle.bundle_key!r} on disk hashes to {raw[:16]}… but the "
                f"admitted manifest inventoried {bundle.raw_sha256[:16]}…. The bytes "
                "changed after the independent verifier read them.")
    ...
    for arm in doc.get("arms") or ():
        ...
        if not arm_key or not arm.get("program_id") \
            ...
            _refuse(GATE_INCOMPLETE_TOPOLOGY,
                    f"bundle {bundle.bundle_key!r} carries an arm with no key, no program "
                    f"or an unknown desired_change: {arm.get('arm_key')!r}/"
                    f"{arm.get('desired_change')!r}", topology=True)
        if not (ranking.get("raw_sha256") and ranking.get("canonical_sha256")):
            _refuse(GATE_INCOMPLETE_TOPOLOGY,
                    f"arm {arm_key!r} binds no content-addressed ranking; an arm whose "
                    "ranking cannot be addressed cannot be replayed.", topology=True)
        arms.append(LoadedArm(
            arm_key=arm_key, lane=bundle.lane, program_id=str(arm["program_id"]),
            desired_change=str(arm["desired_change"]), bundle_key=bundle.bundle_key,
            condition=bundle.condition, from_condition=bundle.from_condition,
...
def _check_arm_topology(arms: list[LoadedArm],
    ...
    if len(programs) != N_PROGRAMS:
        _refuse(GATE_INCOMPLETE_TOPOLOGY,
                ...
                f"{N_PROGRAMS} programs x {len(DESIRED_CHANGES)} desired changes across "
                f"{N_BUNDLES} contexts. Programs are derived from the loaded arms, never "
                "copied from a declared count.", topology=True)

    slots = {a.slot for a in arms}
    ...
    expected = {(b.bundle_key, p, d) for b in bundles for p in programs
                for d in DESIRED_CHANGES}
    missing = sorted(expected - slots)
    if missing:
        _refuse(GATE_INCOMPLETE_TOPOLOGY,
                f"the release resolves {len(slots)} of {len(expected)} arm slots; "
                f"{len(missing)} are missing, e.g. {missing[:3]}. The complete aggregate is "
                f"{N_ARM_SLOTS} logical arms (60 Direct + 120 temporal + 120 pathway) and a "
                "partial release is never admissible.", topology=True)
    if len(slots) != N_ARM_SLOTS:
        _refuse(GATE_INCOMPLETE_TOPOLOGY,
                f"the release resolves {len(slots)} arm slots; the aggregate is exactly "
                f"{N_ARM_SLOTS}.", topology=True)
...
def admit_aggregate(*, manifest_path: str, report_path: str, bundles_root: str,
    ...
    This is the production gate. It replaces ``arm_query.DETACHED_CLONE_MATRIX_GREEN``,
    which was a source-code Boolean and therefore admitted nothing and refused everything —
    it named no manifest, no report, no verifier and no bytes.
    """
    if os.path.realpath(manifest_path) == os.path.realpath(report_path):
        _refuse(GATE_SELF_ADMISSION,
                "the verification report and the aggregate manifest are the SAME file. A "
                "producer does not admit itself; the report must be a separate artifact "
                "from a separate, independent verifier.")
    ...
    try:
        manifest_canonical = content_hash(manifest)
    except CanonicalizationError as exc:
        _refuse(GATE_MANIFEST_UNREADABLE, f"the aggregate manifest is not canonical: {exc}")
        raise  # pragma: no cover - _refuse always raises
    ...
    per_lane = {lane: sum(1 for b in bundles if b.lane == lane) for lane in LANES}
    arms_per_lane = {lane: sum(1 for a in arms if a.lane == lane) for lane in LANES}
    return AdmittedAggregate(
        ...
        counts={"n_bundles": len(bundles), "n_arm_slots": len(arms),
                "bundles_per_lane": per_lane, "arms_per_lane": arms_per_lane,
...
def require_analysis(admitted: AdmittedAggregate) -> AdmittedAggregate:
    ...
    if admitted.artifact_class != ac.ANALYSIS:
        _refuse(GATE_FIXTURE_FIREWALL,
                f"the aggregate declares artifact_class={admitted.artifact_class!r}. Only "
                "an 'analysis' aggregate may be consumed by the analysis path — a fixture "
                "is synthetic data, and a synthetic number on its way to Stage 4 is a "
